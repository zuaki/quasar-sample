...
export default class Calendar extends Vue {
   ...
   */
  // #topic
  // propのデフォルト値は定義した際に格納するのではなく、@Prop()に{default: () => { return "defaultvalue" }}として書く
  // こうしないと親コンポーネントから渡された値が上書きされてしまう
  // ここは通常のJsと同じく、デフォルトメソッドを書く必要がある
  @Prop({ default: () => { return []; } })
  private events!: any[];
  ...
  // ヘッダー部の設定
  @Prop({ default: () => { return { left: "title", center: "", right: "today prev,next" }; } })
  private header!: any;
  // フッター部の設定
  @Prop({ default: () => { return {}; } })
  private footer!: any;
