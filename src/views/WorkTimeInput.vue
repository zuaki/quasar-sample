...
#work-time-input
  .row
    .col-12
      q-card#work-time.shadow-12(color="white" text-color="black")
...
export default class WorkTimeInput extends Vue {
  ...
  /** CSS調整用 class追加 */
  private addCssClass: object[] = [];  // {selector: "", class: ""} の形式を配列で渡す。

  /** CSS調整用 class削除 */
  private removeCssClass: object[] = [];  // {selector: "", class: ""} の形式を配列で渡す。
...
#work-time-input {
  ...
  /deep/ #calendar {
    div.fc-toolbar.fc-header-toolbar > div.fc-left > h2 {
      font-size: 1.8rem;
    }
    ...
    .fc-sun {
      color: red;
      background-color: rgba(234, 199, 205, 0.3);
    }

    .fc-sat {
      color: blue;
      background-color: rgba(165, 209, 244, 0.3);
    }
    ...
    .fc-today {
      background-color: rgba(255, 180, 90, 0.3);
    }
